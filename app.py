...
def construir_informe_estructurado(
    ...
    if tot_news == 0:
        ...
        for categoria, lst in noticias_clas.items():
            if not lst:
                continue
            etiqueta = {"positivas": "✅ Positivas", "negativas": "❌ Negativas", "neutrales": "⚪ Neutrales"}[categoria]
            informe.append(f"  - {etiqueta}:")
            for n in lst[:5]:
                titulo = n.get("title") or "Sin título"
                fuente = n.get("publisher") or n.get("provider") or ""
                link = n.get("link") or ""
                if link:
                    informe.append(f"    - {titulo} ({fuente}) – {link}")
                else:
                    informe.append(f"    - {titulo} ({fuente})")
        informe.append(
